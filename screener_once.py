...
def check_setup(ticker, df, state):
    ...
    # rata-rata volume N hari terakhir (TIDAK termasuk bar hari ini)
    ...
    if setup_match and not already_alerted:
        ...
        # format waktu bar biar lebih enak dibaca (bukan "2026-09-21 13:45:00+07:00")
        try:
            waktu_str = df.index[-1].strftime("%d %b %Y, %H:%M")
        except Exception:
            waktu_str = bar_timestamp
        ...
        msg = (
            ...
            f"Volume     : {format_volume(avg_volume)} : {format_volume(last_volume)}  (avg {AVG_VOLUME_DAYS}D : hari ini)\n"
            f"Timeframe  : {TIMEFRAME.upper()}\n"
            f"Waktu Bar  : {waktu_str} WIB\n"
            f"```"
        )
