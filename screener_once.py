...
TIMEFRAME = "1d"     # Daily — sesuai setup swing kamu (bukan 15m)
LOOKBACK = "6mo"     # data 6 bulan ke belakang, cukup buat EMA21 & ATR14 di timeframe Daily
EMA_FAST = 13
EMA_SLOW = 21
ATR_PERIOD = 10
SUPERTREND_MULTIPLIER = 3.0
VOLUME_MULTIPLIER = 2.0   # "naik minimal 1x" = volume hari ini >= 2x rata-rata volume
AVG_VOLUME_DAYS = 5       # rata-rata volume dihitung dari 5 hari terakhir (bisa diubah 3-7)
...
def get_price_data(ticker, timeframe=TIMEFRAME, lookback=LOOKBACK):
...
def get_batch_price_data(tickers, timeframe=TIMEFRAME, lookback=LOOKBACK, batch_size=50):
...
def format_volume(v):
    """Format angka volume jadi satuan M (juta) atau K (ribu) biar gampang dibaca."""
    if v >= 1_000_000:
        m = v / 1_000_000
        return f"{round(m)} M" if abs(m - round(m)) < 0.05 else f"{m:.1f} M"
    elif v >= 1_000:
        k = v / 1_000
        return f"{round(k)} K" if abs(k - round(k)) < 0.05 else f"{k:.1f} K"
    return f"{v:,.0f}"


def check_setup(ticker, df, state):
    if df is None or len(df) < max(EMA_SLOW, ATR_PERIOD, AVG_VOLUME_DAYS + 1) + 2:
    ...
    trend_intact = (last["ema_fast"] > last["ema_slow"]) and (last["st_direction"] == 1)
    supertrend_bullish = last["st_direction"] == 1
    supertrend_just_flipped = prev["st_direction"] == -1 and last["st_direction"] == 1

    last_volume = last["volume"]
    avg_volume = df["volume"].iloc[-(AVG_VOLUME_DAYS + 1):-1].mean()
    ...
    try:
        tanggal_bar = df.index[-1].strftime("%d %b %Y")
    except Exception:
        tanggal_bar = bar_timestamp
    waktu_kirim = datetime.now(WIB).strftime("%d %b %Y, %H:%M")
    supertrend_label = "Bullish (baru flip)" if supertrend_just_flipped else "Bullish"
    vol_line = f"{format_volume(avg_volume)} : {format_volume(last_volume)}  (avg {AVG_VOLUME_DAYS}D : hari ini)"

    def base_block(vol_status):
        return (
            f"```\n"
            f"Harga      : Rp {price:,.0f}\n"
            f"EMA 13/21  : {last['ema_fast']:,.1f} / {last['ema_slow']:,.1f}\n"
            f"SuperTrend : {supertrend_label}\n"
            f"Volume     : {vol_line}  [{vol_status}]\n"
            f"Timeframe  : {TIMEFRAME.upper()}\n"
            f"Waktu Bar  : {tanggal_bar} (Daily)\n"
            f"Dikirim    : {waktu_kirim} WIB\n"
            f"```"
        )

    ticker_state = state.get(ticker, {})

    # === Kasus 1: golden cross BARU terjadi hari ini ===
    if golden_cross_now and supertrend_bullish:
        if volume_confirmed:
            # Cross + volume langsung kompak hari yang sama -> 1 notif lengkap
            msg = f"🟢 *SINYAL TERKONFIRMASI: {kode}*\n" + base_block("✅ sudah konfirmasi")
            send_telegram_alert(msg)
            state[ticker] = {"cross_bar": bar_timestamp, "confirmed": True}
            print(f"[ALERT] {ticker} -> confirmed (langsung, cross+volume bareng)")
        else:
            # Cross duluan, volume belum -> kirim sinyal awal, tandai masih "pending"
            msg = f"🔵 *SINYAL AWAL: {kode}*\n" + base_block("⏳ belum capai syarat")
            send_telegram_alert(msg)
            state[ticker] = {"cross_bar": bar_timestamp, "confirmed": False}
            print(f"[ALERT] {ticker} -> early signal terkirim, nunggu volume")
        return

    # === Kasus 2: ada cycle pending dari hari sebelumnya (sudah dapat sinyal awal) ===
    if ticker_state and not ticker_state.get("confirmed", True):
        if trend_intact:
            if volume_confirmed:
                msg = f"🟢 *SINYAL TERKONFIRMASI: {kode}*\n" + base_block("✅ sudah konfirmasi")
                send_telegram_alert(msg)
                ticker_state["confirmed"] = True
                state[ticker] = ticker_state
                print(f"[ALERT] {ticker} -> confirmed (susulan, volume baru capai syarat)")
            else:
                print(f"[INFO] {ticker}: masih pending, volume belum capai syarat (ratio={volume_ratio:.2f}x)")
        else:
            # Trend udah rusak sebelum volume sempat konfirmasi -> batalin cycle diam-diam
            del state[ticker]
            print(f"[INFO] {ticker}: cycle dibatalkan, trend sudah rusak sebelum volume konfirmasi")
        return
